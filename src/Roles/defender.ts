const run = function (creep) {
    ...
    if(creep.room.memory.danger) {
        ...
        if(enemyCreeps.length > 0) {
            ...
            const rampartToMan = creep.room.memory.rampartToMan;

            const rampart:any = Game.getObjectById(rampartToMan);

            if(creep.body.length <= 3 || creep.body.length % 3 == 0) {
                const rampartDefenders = creep.room.find(FIND_MY_CREEPS, {filter: creep => creep.memory.role == "RampartDefender"});
            ...
            if(creep.pos.isNearTo(closestEnemyCreep)) {
                ...
                const target = creep.room.memory.attack_target
